...
| 9   | `kseq_test` second-stage trim — what length?                      | `integrated.sh` reads from a `length` file in the working directory.                                                                                                                                             | **Resolved: 42 bp.** Confirmed from config files (`fastq_sequence_length: 42`). Source code, header, build script, and pre-compiled binary all in repo at `references/cutruntools/`. |
| 10  | Trimmomatic adapter file                                          | Need a copy of `Truseq3.PE.fa` from the lab instance for the clone's trimming module.                                                                                                                            | **Resolved.** All four adapter files in repo at `references/cutruntools/adapters/`. Ship with the clone. |
...
| 14  | Lab scripts collection status                                      | TODO.md listed scripts to collect from lab instance (diffbind.R, normalization.r, etc.)                                                                                                                          | **Resolved: ALL scripts already in repo** under `references/`. Complete inventory in `cleave-spec-decisions.md` §3 Q9. |
| 15  | SNAP-CUTANA spike-in barcode sequences                             | Previously considered proprietary/unavailable.                                                                                                                                                                    | **Resolved: All 32 barcode sequences in repo** at `references/media_misc/k_metstat_script.sh`. Implement spike-in QC in Phase 3. See `cleave-spec-decisions.md` §8. |
