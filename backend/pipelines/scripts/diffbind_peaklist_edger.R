...
#
# DiffBind differential peak analysis — custom peakset mode with edgeR backend + TMM normalization.
# Fixed version of references/DPA/diffbind_peaklist_edgeR.R with bug corrections:
#   2. Malformed print() completion message
#   3. Missing dev.off() between PNG and SVG device opens
# Note: Bug 1 (write.csv) was already fixed in the edgeR variant.
# Note: Heatmap plots remain commented out (matching lab reference script).
...
db <- dba.count(db, peaks=peakset, summits=200, filter=0)

# edgeR TMM normalization and differential analysis
db <- dba.normalize(db,
                    method    = DBA_NORM_TMM,
                    library   = DBA_LIBSIZE_FULL,
                    normalize = DBA_NORM_NATIVE)
db <- dba.score(db, score = DBA_SCORE_TMM_READS_EFFECTIVE)
db <- dba.contrast(db, categories = DBA_CONDITION, minMembers = 2)
dbAnalyze <- dba.analyze(db, method = DBA_EDGER)
...
# Heatmap plots disabled for edgeR mode (matching lab reference)
