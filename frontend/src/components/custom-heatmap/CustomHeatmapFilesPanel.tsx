// frontend/src/components/custom-heatmap/CustomHeatmapFilesPanel.tsx
...
import { CUSTOM_HEATMAP_FILE_CATEGORIES } from '@/lib/constants';
import { formatBytes } from '@/lib/utils';

interface CustomHeatmapFilesPanelProps {
  jobId: number;
}

type HeatmapFileCategory = (typeof CUSTOM_HEATMAP_FILE_CATEGORIES)[number]['value'];

export function CustomHeatmapFilesPanel({ jobId }: CustomHeatmapFilesPanelProps) {
  const [selectedCategory, setSelectedCategory] = useState<HeatmapFileCategory>(
    CUSTOM_HEATMAP_FILE_CATEGORIES[0].value,
  );
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set());

  const { data: outputs, isLoading } = useJobOutputs(jobId, selectedCategory);

  const categoryInfo = CUSTOM_HEATMAP_FILE_CATEGORIES.find((c) => c.value === selectedCategory);

  function handleCategoryChange(e: React.ChangeEvent<HTMLSelectElement>) {
    setSelectedCategory(e.target.value as HeatmapFileCategory);
