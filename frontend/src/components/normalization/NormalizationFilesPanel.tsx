...
import { NORMALIZATION_FILE_CATEGORIES } from '@/lib/constants';
import { formatBytes } from '@/lib/utils';

interface NormalizationFilesPanelProps {
  jobId: number;
}

type NormalizationFileCategory = (typeof NORMALIZATION_FILE_CATEGORIES)[number]['value'];

export function NormalizationFilesPanel({ jobId }: NormalizationFilesPanelProps) {
  const [selectedCategory, setSelectedCategory] = useState<NormalizationFileCategory>(
    NORMALIZATION_FILE_CATEGORIES[0].value,
  );
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set());

  const { data: outputs, isLoading } = useJobOutputs(jobId, selectedCategory);

  const categoryInfo = NORMALIZATION_FILE_CATEGORIES.find(
    (c) => c.value === selectedCategory,
  );

  function handleCategoryChange(e: React.ChangeEvent<HTMLSelectElement>) {
    setSelectedCategory(e.target.value as NormalizationFileCategory);
    ...
    <Card>
      <div className="mb-4 flex items-start gap-4">
        <div className="shrink-0">
          ...
          <select
            id="normalization-file-category"
          ...
          >
            {NORMALIZATION_FILE_CATEGORIES.map((cat) => (
