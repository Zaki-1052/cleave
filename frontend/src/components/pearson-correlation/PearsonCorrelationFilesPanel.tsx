...
import { PEARSON_CORRELATION_FILE_CATEGORIES } from '@/lib/constants';
import { formatBytes } from '@/lib/utils';

interface PearsonCorrelationFilesPanelProps {
  jobId: number;
}

type PearsonFileCategory = (typeof PEARSON_CORRELATION_FILE_CATEGORIES)[number]['value'];

export function PearsonCorrelationFilesPanel({ jobId }: PearsonCorrelationFilesPanelProps) {
  const [selectedCategory, setSelectedCategory] = useState<PearsonFileCategory>(
    PEARSON_CORRELATION_FILE_CATEGORIES[0].value,
  );
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set());

  const { data: outputs, isLoading } = useJobOutputs(jobId, selectedCategory);

  const categoryInfo = PEARSON_CORRELATION_FILE_CATEGORIES.find(
    (c) => c.value === selectedCategory,
  );

  function handleCategoryChange(e: React.ChangeEvent<HTMLSelectElement>) {
    setSelectedCategory(e.target.value as PearsonFileCategory);
    ...
    <Card>
      <div className="mb-4 flex items-start gap-4">
        <div className="shrink-0">
          ...
          >
            {PEARSON_CORRELATION_FILE_CATEGORIES.map((cat) => (
