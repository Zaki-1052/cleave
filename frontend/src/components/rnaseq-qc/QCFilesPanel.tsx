// frontend/src/components/rnaseq-qc/QCFilesPanel.tsx
...
interface QCFilesPanelProps {
  jobId: number;
}

type QCFileCategory = (typeof RNASEQ_QC_FILE_CATEGORIES)[number]['value'];

export function QCFilesPanel({ jobId }: QCFilesPanelProps) {
  const [selectedCategory, setSelectedCategory] = useState<QCFileCategory>(
    RNASEQ_QC_FILE_CATEGORIES[0].value,
  );
  ...
  const categoryInfo = RNASEQ_QC_FILE_CATEGORIES.find((c) => c.value === selectedCategory);

  function handleCategoryChange(e: React.ChangeEvent<HTMLSelectElement>) {
    setSelectedCategory(e.target.value as QCFileCategory);
